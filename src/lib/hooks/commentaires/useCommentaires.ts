...
// src/hooks/commentaires/useCommentaires.ts
...
interface UseCommentairesOptions {
  initialQuery?: QueryCommentairesDto;
  autoFetch?: boolean;
}

export const useCommentaires = (options: UseCommentairesOptions = {}) => {
  ...
  const fetchCommentaires = async (newQuery?: QueryCommentairesDto) => {
    ...
    try {
      const result = await commentairesApi.getCommentaires(newQuery || query);
